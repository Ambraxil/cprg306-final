...
export default function ListPage() {
  ...
  const [selectedTask, setSelectedTask] = useState(null);
    ...
    if (!user) return;
    const docId = await addList(user.uid, lists);
  ...
  const handleAddTask = async (task) => {
    if (!user) return;
    const docId = await addTask(user.uid, selectedList, task);
  ...
  const handleDeleteTask = async (taskId) => {
    ...
    const downloadTxtFile = () => {
      let inputText = tasks.map(task => 
        `Title: ${task.title}, Details: ${task.details || "No details"}, Start: ${task.startTime || "Not set"}, End: ${task.endTime || "Not set"}, Completed: ${task.completed ? "Yes" : "No"}`
      ).join('\n');
  ...
  if (selectedList == null) {
    return (
    <div className="justify-center min-h-screen bg-gray-300 text-blue-900 p-6">
      <header className="w-full text-left pb-4 mb-6 border-b-4 border-blue-900">
        <h1 className="text-3xl font-bold">Swift List</h1>
        </header>
